...
@given(u'att man befinner sig på webbsidan Läslistan')
def step_given__on_the_webpage(context):
    context.laslistan = Laslistan(context.page)
    context.page.goto(context.base_url)


@then(u'bör man se att titeln på webbsidan är "{title}"')
...
@then(u'bör man kunna se att det finns "{button_count:d}" st navigationsknappar på webbsidan')
...
@then(u'bör man kunna se att det är rätt namn och testid på vardera navigationsknapp på webbsidan')
...
@when(u'när man klickar på en navigeringsknapp med ett specifikt test-id: "{testid}"')
def step_when__click_on_navbutton(context, testid):
    context.laslistan.navigation_button_click(testid)


@then(u'bör man se att den aktuella navigeringknappen, "{testid}", blivit deaktiverad')
